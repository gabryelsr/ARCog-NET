...
def trajetoria_cpp(id, sender, uav):
    ...
    if uav==0:
        ...
        for i in range(12):
            ...
            h=random.uniform(12,18)
            H.append(h)
            L.append(l)

        pontos=[[L[0],L[0],H[0]],[L[1]+5*L[1],L[1],H[1]],[L[2]+5*L[2],L[2]+L[2],H[2]],[L[3],L[3]+L[3],H[3]],[L[4],L[4]+2*L[4],H[4]],
        [L[5]+5*L[5],L[5]+2*L[5],H[5]],[L[6]+5*L[6],L[6]+3*L[6],H[6]],[L[7],L[7]+3*L[7],H[7]],[L[8],L[8]+4*L[8],H[8]],[L[9]+5*L[9],L[9]+4*L[9],H[9]],
        [L[10]+5*L[10],L[10]+5*L[10],H[10]], [L[11],L[11]+5*L[11],H[11]]]
    else:
        if uav%2 == 0:
            L=[]
            H=[]
            for i in range(12):
                l=random.uniform(3,5)+2*uav
                h=random.uniform(12,18)
                H.append(h)
                L.append(l)
            ...
            for i in range(len(pontos)):
                pontos[i][0] = pontos[i][0]+15*uav
        else:
            L=[]
            H=[]
            for i in range(12):
                l=-1*random.uniform(3,5)+2*uav
                h=random.uniform(12,18)
                H.append(h)
                L.append(l)
            ...
            [L[10]+5*L[10],L[10]+5*L[10],H[10]], [L[11],L[11]+5*L[11],H[11]]]
            for i in range(len(pontos)):
                pontos[i][0] = -1*pontos[i][0]-15*uav
    
    t=datetime.now()
    t=float(t.strftime("%Y%m%d%H%M%S.%f"))
    msg = [[id], [2], [sender], [uav], [t], pontos] #2 é o código para configurar trajetória

    x = [sublista[0] for sublista in pontos]
    y = [sublista[1] for sublista in pontos]
    plt.plot(x, y)
    plt.xlabel("X (m)")
    plt.ylabel("Y (m)")
    plt.title("Navigated trajectory of UAV "+str(uav))
    plt.grid()
    plt.savefig('/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/missoes/dados/trajetoria_calculada_city'+str(uav)+'.png')
    
    return msg

def ponto_transmissao(id, sender, uav):

    x = random.uniform(20,500)
    y = random.uniform(20,800)
    h = random.uniform(50,70)
    ponto=[[x, y, h]]
    
    t=datetime.now()
    t=float(t.strftime("%Y%m%d%H%M%S.%f"))
    msg = [[id], [2], [sender], [uav], [t], ponto] #2 é o código para configurar trajetória

    x = [sublista[0] for sublista in ponto]
    y = [sublista[1] for sublista in ponto]

    plt.rcParams["figure.figsize"] = [7.00, 3.50]
    plt.rcParams["figure.autolayout"] = True
    im = plt.imread("/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/missoes/dados/cidade.png")
    fig, ax = plt.subplots()
    im = ax.imshow(im, extent=[0, 500, 0, 700])
    ax.plot(x,y, 'o', label="UAV"+str(uav))
    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
    ax.set_title("Destined point of UAV "+str(uav))
    plt.savefig('/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/missoes/dados/ponto_city'+str(uav)+'.png')
    
    return msg

def trajetoria_indoor(id, sender, uav):

    pz = 1.5

    #pesos base
    t1 = [[1.5, 1.5, pz], [1.5, 4.35, pz]]
    t2 = [[1.5, 1.5, pz], [4.1, 1.5, pz], [4.1, 4.35, pz]]
    
    #pesos complementares
    t3 = [[0.62, 4.35, pz], [0.62, 6.6, pz], [1.25, 6.6, pz], [1.25, 7.5, pz], [1.25, 6.6, pz], [2.6, 6.6, pz]]
    t4 = [[3.8, 4.35, pz], [3.8, 6.6, pz], [5.0, 6.6, pz], [5.0, 8.0, pz], [5.0, 6.6, pz], [6.1, 6.6, pz]]
    t5 = [[7.2, 4.55, pz], [7.25, 6.55, pz], [8.4, 6.55, pz], [8.4, 7.5, pz], [8.4, 6.55, pz], [9.4, 6.55, pz]]
    t6 = [[7.2, 4.55, pz], [9.0, 4.55, pz], [9.0, 1.5, pz], [7.2, 1.5, pz], [7.2, 3.0, pz]]
    
    i = int(random.uniform(1,3))
    if i == 1:
        j = int(random.uniform(3,7))
        if j == 3:
            traj = t1+t3
        elif j == 4:
            traj = t1+t4
    ...
    t=datetime.now()
    t=float(t.strftime("%Y%m%d%H%M%S.%f"))
    msg = [[id], [2], [sender], [uav], [t], traj] #2 é o código para configurar trajetória

    x = [sublista[0] for sublista in traj]
    y = [sublista[1] for sublista in traj]
    z = [sublista[2] for sublista in traj]

    plt.plot(x,y)
    plt.xlabel("X (m)") 
    plt.ylabel("Y (m)")
    plt.title("Calculated trajectory of UAV "+str(uav))
    plt.grid()
    plt.savefig('/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/experimento/dados/calc_indoor_uav'+str(uav)+'.png')
    plt.clf()
    ...
    ax.set_ylabel('Y (m)')
    ax.set_zlabel('Z (m)')
    ax.set_title("Navigated 3D trajectory of UAV "+str(uav))
    ax.view_init(-140, 45)
    plt.savefig('/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/experimento/dados/3Dcalc_indoor_uav'+str(uav)+'.png')
    
    return msg

def trajetoria_lacea(id, sender, uav):
    #cw = negativo
    #ccw = positivo
    #[yaw(t+1), dist(t+1), ang(t), x(t), y(t), z(t)]

    #pesos tello
    with open('pesos_tello.pkl', 'rb') as f:
        pesos = pickle.load(f)
    ...
    if uav==1:
        ...
        cog = [7000] #id de nada a fazer
    
    t=datetime.now()
    t=float(t.strftime("%Y%m%d%H%M%S.%f"))
    msg = [[id], [2], [sender], [uav], [t], cog] #2 é o código para configurar trajetória
    
    return msg

def trajetoria_lacea_exp(id, sender, uav):
    #cw = negativo
    #ccw = positivo
    #[yaw(t+1), dist(t+1), ang(t), x(t), y(t), z(t)]
    pz = 1.5
...
class Cloud():
    ...
    def cloud_loop(self, cluster1, cluster2, cloudid):
        ...
        #L=4
        #h=15
        #pontos=[[L,L,h],[L+5*L,L,h],[L+5*L,L+L,h],[L,L+L,h],[L,L+2*L,h],
        #[L+5*L,L+2*L,h],[L+5*L,L+3*L,h],[L,L+3*L,h],[L,L+4*L,h],[L+5*L,L+4*L,h],[L+5*L,L+5*L,h], [L,L+5*L,h]]

        if self.rede_config == False:
        ...
        while True:
            for cluster in estrutura:
                for uav in cluster:
                    ...
                    #pontos=trajetoria_cpp(id, cloudid, uav)
                    #pontos = ponto_transmissao(id, cloudid, uav)
                    #pontos = trajetoria_indoor(id, cloudid, uav)
                    pontos = trajetoria_lacea_exp(id, cloudid, uav)
                    #pontos = trajetoria_lacea(id, cloudid, uav)
                    trajetorias.append(pontos[-1])
                    if cluster == estrutura[-1] and uav == cluster[-1]:
                        with open("/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/experimento/dados/trajetorias_lacea.json", 'w') as f:
                    ...
                    try:
                        noventrada = {'LAYER': self.layer, 'NODE': cloudid, 'REC_TIME': t, 'RX/TX': 'TX',
                                'ID': pontos[0][0],'TYPE': pontos[1][0],'SENDER': pontos[2][0],
                                'RECEIVER': pontos[3][0],'MSG_TIME': pontos[4][0], 'CONTENT': pontos[-1]}
                        self.bd.loc[len(self.bd)] = noventrada
                    except:
                        pass
                    time.sleep(0.1)
                    self.le_coordenador(uav)
                    if self.coor_msg is not None:
                        print("Mensagem recebida!")
                        print(self.coor_msg[-1][0])
        ...
        #salvar dataframe
        #self.bd.to_excel('/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/missoes/dados/banco_de_dados_cloud_city.xlsx')"""
