...
numero_uav = 4
#ponto_acesso = 'wlx00e0470dfb0b' #1 TELLO-C41352
#ponto_acesso = 'wlx00e0450dadc4' #2 TELLO-D066AE
#ponto_acesso = 'wlx00e0410d136b' #3 TELLO-DC04EE
ponto_acesso = 'wlxc02a00400255' #4 TELLO-C46B20
#ponto_acesso = 'wlx00e0470dfb22' #5 TELLO-999820
#ponto_acesso = 'wlx00e0450dacf4' #6 TELLO-C34472
...
# float32 yaw_rate
	...
	def publica_coor(self, id, tipo, rec, pos):
		#rate = rospy.Rate(10)
		#coor="c_uav_"
		if not rospy.is_shutdown() and self.local_pose is not None:
			self.current_pos_x = pos[0] #self.local_pose.pose.position.x
			self.current_pos_y = pos[1] #self.local_pose.pose.position.y
			self.current_pos_z = pos[2] #self.local_pose.pose.position.z
			self.current_heading = pos[3]
			t=datetime.now()
			t=float(t.strftime("%Y%m%d%H%M%S.%f"))
	...
	def publica_agente(self, id, tipo, rec, pos):
		#rate = rospy.Rate(10)
		#agen="a_uav_"
		if not rospy.is_shutdown() and self.local_pose is not None:
			self.current_pos_x = pos[0] #self.local_pose.pose.position.x
	...
	def rede(self, id, tipo, rec, pos): #configura ou reconfigura rede
		if self.cluster_local == []:
				print("Rede ainda não definida --iniciando configuração")
		else:
			print("Rede definida")
			...
			if self.uav_num == self.cluster_local[0]:
				print("Sou coordenador!")
				self.publica_coor(id, tipo, rec, pos)
				for a in self.cluster_local[1:]:
					self.le_agente(a)
					print(f"Mensagem do agente {a}: {self.agente_msg}")
				for c in self.coordenadores:
					self.le_coordenador(c)
					print(f"Mensagem do coordenador {c}: {self.coor_msg}")
				...
				print("Sou agente!")
				self.publica_agente(id, tipo, rec, pos)
				self.le_coordenador(self.cluster_local[0])
				print(f"Mensagem do coordenador: {self.coor_msg}")
				#print(self.cloud_msg)
				if self.cloud_msg[1]==[2]:
						if self.cloud_msg[3]==[self.uav_num]:
							self.pontos = self.cloud_msg[-1]
	...
	def navegue(self, status_decolagem,j=0):
		...
		while self.arm_state and self.offboard_state and (rospy.is_shutdown() is False):
			...
			if self.pontos == None:
				...
				if j < len(self.pontos):
					xt=self.pontos[j][0]-(1.5 + 0.5*numero_uav)
					yt=self.pontos[j][1]-1.5
					zt=self.pontos[j][2]
					...
					if dist_tgt <= precisao:
						...
						if j>=len(self.pontos):
							...
							if self.plotou == False:
								...
								plt.grid()
								plt.savefig('/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/experimento/dados/trajetoria_uav'+str(self.uav_num)+'.png')
								plt.clf()
								trajetoria = []
								for i in range(len(navx)): #matriz historico de trajetorias
									trajetoria.append([navx[i], navy[i], navz[i]])
								with open("/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/experimento/dados/trajetoria_uav"+str(self.uav_num)+".json", 'w') as f:
								...
								ax.view_init(-140, 45)
								plt.savefig('/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/experimento/dados/trajetoria3D_uav'+str(self.uav_num)+'.png')
								self.plotou = True
				if self.image is not None:
		...
		#salvar dataframe
		self.bd.to_excel('/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/experimento/dados/banco_de_dados_uav'+str(self.uav_num)+'.xlsx')
		trajetoria = []
		for i in range(len(navx)): #matriz historico de trajetorias
			trajetoria.append([navx[i], navy[i], navz[i], navheading[i], navt[i]])
		with open("/home/gabryelsr/src/Firmware/Tools/simulation/gazebo-classic/sitl_gazebo-classic/experimento/dados/banco_trajetoria_uav"+str(self.uav_num)+".json", 'w') as f:
			json.dump(trajetoria, f, indent=2)

	def arcog_tello_config(self):
		drone = None
		print("PONTO 2")
		try:
			drone = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
			drone.setsockopt(socket.SOL_SOCKET, socket.SO_BINDTODEVICE, ponto_acesso.encode())
			drone.sendto('command'.encode(), (tello_ip, udp_port))
			time.sleep(1)
			drone.sendto('speed 50'.encode(), (tello_ip, udp_port)) #configurando velocidade a 50cm/s
			time.sleep(2)
			drone.sendto('command'.encode(), (tello_ip, udp_port))
			time.sleep(1)
			drone.sendto('takeoff'.encode(), (tello_ip, udp_port)) #decolar
			time.sleep(10)
			drone.sendto('command'.encode(), (tello_ip, udp_port))
			time.sleep(1)
			drone.sendto('up 70'.encode(), (tello_ip, udp_port)) #subir 50cm
			time.sleep(10)
	...
	def arcog_tello_nav(self, yaw, fw, drone):
		tsleep = 5
		if yaw != 0:
			if yaw > 0:
				print(f"Drone {numero_uav} girando {abs(yaw)} no sentido anti-horario")
				drone.sendto('command'.encode(), (tello_ip, udp_port))
				time.sleep(1)
				drone.sendto(('ccw '+ str(abs(yaw))).encode(), (tello_ip, udp_port)) # gira sentido anti horario
				time.sleep(tsleep)
			else:
				print(f"Drone {numero_uav} girando {abs(yaw)} no sentido horario")
				drone.sendto('command'.encode(), (tello_ip, udp_port))
				time.sleep(1)
				print('cw '+ str(abs(yaw)))
				drone.sendto(('cw '+ str(abs(yaw))).encode(), (tello_ip, udp_port)) # gira sentido horario
				time.sleep(tsleep)
		if fw != 0:
			drone.sendto('command'.encode(), (tello_ip, udp_port))
			print(f"Drone {numero_uav} avançando {fw}m")
			drone.sendto(('forward '+ str(fw*100)).encode(), (tello_ip, udp_port)) # avança (o comando de avanço é em cm e o calculo do fw é em m)
			time.sleep(int(fw*5)) #aguarda o tempo do avanço

	def arcog_tello_lacea(self, numero_uav, status_decolagem):

		#cw = negativo
		#ccw = positivo
		#[yaw(t+1), dist(t+1), ang(t), x(t), y(t), z(t)]

		#drone = self.arcog_tello_config()

		id = 0

		"""if numero_uav==1:
			xt, yt, zt, wt = 1.5, 1.5, 1.5, 0
		elif numero_uav==2:
			xt, yt, zt, wt = 2, 1.5, 1.5, 0
		elif numero_uav==3:
			xt, yt, zt, wt = 2.5, 1.5, 1.5, 0
		elif numero_uav==3:
			xt, yt, zt, wt = 3.0, 1.5, 1.5, 0
		elif numero_uav==4:
			xt, yt, zt, wt = 3.5, 1.5, 1.5, 0
		elif numero_uav==5:
			xt, yt, zt, wt = 4.0, 1.5, 1.5, 0
		elif numero_uav==6:
			xt, yt, zt, wt = 4.6, 1.5, 1.5, 0
		else:
			xt, yt, zt, wt = 0, 0, 1.5, 0"""
		...
		if status_decolagem == True:
			while self.sou_coor == False or self.sou_agente == False:
				print("PONTO 1")
				pos = [posx[-1], posy[-1], posz[-1], posw[-1]]
				self.rede(id, 9000, 9000, pos)
				id = id+1
				drone = self.arcog_tello_config()
		...
		while True:
			...
			if self.pontos is not None:
				self.ida = self.pontos[0]
				self.volta = self.pontos[1]
				
				traje = self.ida if self.idast and not self.voltast else self.volta
				print("Trajetória atual:", "Ida" if self.idast else "Volta")
				print(traje)
                
				if self.sou_coor == True:
					for i in range(len(traje)):
						print("PONTO 4")
						print("Drone " + str(numero_uav) + " navegando para o ponto " + str(i))
						self.arcog_tello_nav(traje[i][0], traje[i][1], drone)
						xt=traje[i][3]
						posx.append(xt)
						yt=traje[i][4]
						posy.append(yt)
						...
						self.rede(id, 9000, 9000, pos)
					print("Invertendo trajetoria")
					self.inverter_trajetoria()	
	...
	#rospy.init_node("offboard_node")
		...
		while self.arm_state and self.offboard_state and (rospy.is_shutdown() is False):
			...
			if j > len(pontos):
				...
				log_etapa1.to_csv('/home/gabryelcefetrj/src/Firmware/Tools/sitl_gazebo/experimento/log_pratico_etapa1.csv', index=False)
		...
		if status_decolagem==True:
			...
			while self.arm_state and self.offboard_state and (rospy.is_shutdown() is False):
				...
				if self.camera_msg=='Parando Etapa 2!' or self.placa_msg=='Parando Etapa 2!':
					...
					log_etapa2.to_csv('/home/gabryelcefetrj/src/Firmware/Tools/sitl_gazebo/experimento/log_etapa2.csv', index=False)
	...
	def segue_alvo(self, status_decolagem):
		...
		while self.arm_state and self.offboard_state and (rospy.is_shutdown() is False):	
			...
			self.cur_target_pose = self.construct_target(xt,yt,zt,yawt)
			self.local_target_pub.publish(self.cur_target_pose)
	def inverter_trajetoria(self):
		self.idast = not self.idast
		self.voltast = not self.voltast
		print(f"Inversão feita: idast={self.idast}, voltast={self.voltast}")
...
if __name__ == '__main__':
	...
	while True:
		...
		#con.fixa(decola, h)
		#con.navegue(decola)
		con.arcog_tello_lacea(numero_uav, decola)
		#con.sitl()
		#con.publica_posicao()
